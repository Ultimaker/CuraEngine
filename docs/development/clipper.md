...
* [CMakeDeps](https://docs.conan.io/en/latest/reference/conanfile/tools/cmake/cmakedeps.html): generates information about where the **clipper** library and its dependencies  are installed together with other information like version, flags, and directory data or configuration. CMake will use this files when you invoke ``find_package()`` in your *CMakeLists.txt*.
...
Declare these generators in your **conanfile.txt** along with your **clipper** dependency like:
...
To use **clipper** in a simple CMake project with this structure:
...
Your **CMakeLists.txt** could look similar to this, using the global **clipper::clipper** CMake's target:

```cmake
cmake_minimum_required(VERSION 3.15)
project(clipper_project CXX)
...
target_link_libraries(${PROJECT_NAME} clipper::clipper)
```

<br>

To install **clipper/6.4.2**, its dependencies and build your project, you just have to do:
...
## Using clipper with Visual Studio
...
* [MSBuildDeps](https://docs.conan.io/en/latest/reference/conanfile/tools/microsoft.html#msbuilddeps): generates the **conandeps.props** properties file with information about where the **clipper** library and its dependencies  are installed together with other information like version, flags, and directory data or configuration.
...
## Using clipper with Autotools and pkg-config
...
* [PkgConfigDeps](https://docs.conan.io/en/latest/reference/conanfile/tools/gnu/pkgconfigdeps.html): generates the **polyclipping.pc** file (and the ones corresponding to **clipper** dependencies) with information about the dependencies that can be later used by the **pkg-config** tool pkg-config to collect data about the libraries Conan installed.
...
# You will have to set the PKG_CONFIG_PATH to where Conan created the polyclipping.pc file
$ export CPPFLAGS="$CPPFLAGS $(pkg-config --cflags clipper)"
$ export LIBS="$LIBS $(pkg-config --libs-only-l clipper)"
$ export LDFLAGS="$LDFLAGS $(pkg-config --libs-only-L --libs-only-other clipper)"
...
## Exposed header files for clipper
