...
* [CMakeDeps](https://docs.conan.io/en/latest/reference/conanfile/tools/cmake/cmakedeps.html): generates information about where the **llvm-openmp** library and its dependencies  are installed together with other information like version, flags, and directory data or configuration. CMake will use this files when you invoke ``find_package()`` in your *CMakeLists.txt*.
...
Declare these generators in your **conanfile.txt** along with your **llvm-openmp** dependency like:
...
To use **llvm-openmp** in a simple CMake project with this structure:
...
Your **CMakeLists.txt** could look similar to this, using the global **llvm-openmp::llvm-openmp** CMake's target:

```cmake
cmake_minimum_required(VERSION 3.15)
project(llvm-openmp_project CXX)
...
target_link_libraries(${PROJECT_NAME} llvm-openmp::llvm-openmp)
```

<br>

To install **llvm-openmp/12.0.1**, its dependencies and build your project, you just have to do:
...
## Using llvm-openmp with Visual Studio
...
* [MSBuildDeps](https://docs.conan.io/en/latest/reference/conanfile/tools/microsoft.html#msbuilddeps): generates the **conandeps.props** properties file with information about where the **llvm-openmp** library and its dependencies  are installed together with other information like version, flags, and directory data or configuration.
...
## Using llvm-openmp with Autotools and pkg-config
...
* [AutotoolsDeps](https://docs.conan.io/en/latest/reference/conanfile/tools/gnu/autotoolsdeps.html): generates the **conanautotoolsdeps.sh/bat** script with information about where the **llvm-openmp** library and its dependencies  are installed together with other information like version, flags, and directory data or configuration. This is done setting some enviroment variables for Autotools like: ``LIBS``, ``CPPFLAGS``,``CXXFLAGS``, ``CFLAGS`` and ``LDFLAGS``.  It will also generate a ``deactivate_conanautotoolsdeps.sh/bat`` so you can restore your environment.
...
* [PkgConfigDeps](https://docs.conan.io/en/latest/reference/conanfile/tools/gnu/pkgconfigdeps.html): generates the **llvm-openmp.pc** file (and the ones corresponding to **llvm-openmp** dependencies) with information about the dependencies that can be later used by the **pkg-config** tool pkg-config to collect data about the libraries Conan installed.
...
# You will have to set the PKG_CONFIG_PATH to where Conan created the llvm-openmp.pc file
$ export CPPFLAGS="$CPPFLAGS $(pkg-config --cflags llvm-openmp)"
$ export LIBS="$LIBS $(pkg-config --libs-only-l llvm-openmp)"
$ export LDFLAGS="$LDFLAGS $(pkg-config --libs-only-L --libs-only-other llvm-openmp)"
...
## Exposed header files for llvm-openmp
