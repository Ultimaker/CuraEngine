...
BENCHMARK_DEFINE_F( InfillTest, Infill_generate_connect )(benchmark::State& st)
{
    ...
    for ( auto _ : st )
    {
        ...
        infill.generate( result_paths, result_polygons, result_lines, settings, 0, debug::SectionType::WALL, nullptr, nullptr );
    }
}

BENCHMARK_REGISTER_F( InfillTest, Infill_generate_connect )->ArgsProduct( {{ true, false },
                                                                           { 400,  800, 1200 }} )->Unit( benchmark::kMillisecond );
} // namespace cura
