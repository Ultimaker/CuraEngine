...
public:
    ...
    */
     ...
     * \param polygons The polygons from which to get the polygon
     * \param poly_idx The index in \p polygons of the polygon to add
     * \param start_point_idx The index of the starting vertex of the polygon
     ...
     */
    void addPolygon(const Polygons& polygons, unsigned int poly_idx, unsigned int start_point_idx, const GCodePathConfig& config, PolygonFlowAdjuster* flow_adjuster = nullptr, coord_t wall_0_wipe_dist = 0, bool spiralize = false, float flow_ratio = 1.0, bool always_retract = false);
     ...
     * \param always_retract Whether to force a retraction when moving to the start of the polygon (used for outer walls)
     */
    void addPolygonsByOptimizer(const Polygons& polygons, const GCodePathConfig& config, PolygonFlowAdjuster* flow_adjuster = nullptr, const ZSeamConfig& z_seam_config = ZSeamConfig(), coord_t wall_0_wipe_dist = 0, bool spiralize = false, float flow_ratio = 1.0, bool always_retract = false);
