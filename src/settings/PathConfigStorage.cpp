...
PathConfigStorage::MeshPathConfigs::MeshPathConfigs(
    ...
    , skin_config(
          ...
          SpeedDerivatives{ mesh.settings.get<Velocity>("speed_topbottom"),
    ...
    , bridge_skin_config( // use bridge skin flow, speed and fan
          ...
          SpeedDerivatives{ mesh.settings.get<Velocity>("bridge_skin_speed"),
                            mesh.settings.get<Acceleration>("acceleration_topbottom"),
                            mesh.settings.get<Velocity>("jerk_topbottom") },
          true // is_bridge_path
          ,
    ...
    , bridge_skin_config2( // use bridge skin 2 flow, speed and fan
          ...
          SpeedDerivatives{ mesh.settings.get<Velocity>("bridge_skin_speed_2"),
                            mesh.settings.get<Acceleration>("acceleration_topbottom"),
                            mesh.settings.get<Velocity>("jerk_topbottom") },
          true // is_bridge_path
          ,
    ...
    , bridge_skin_config3( // use bridge skin 3 flow, speed and fan
          ...
          SpeedDerivatives{ mesh.settings.get<Velocity>("bridge_skin_speed_3"),
                            mesh.settings.get<Acceleration>("acceleration_topbottom"),
                            mesh.settings.get<Velocity>("jerk_topbottom") },
          true // is_bridge_path
          ,
...
PathConfigStorage::PathConfigStorage(const SliceDataStorage& storage, const LayerIndex& layer_nr, const coord_t layer_thickness)
    ...
    , raft_base_config(
          ...
          Ratio(1.0),
          SpeedDerivatives{ raft_base_train.settings.get<Velocity>("raft_base_speed"),
                            raft_base_train.settings.get<Acceleration>("raft_base_acceleration"),
                            raft_base_train.settings.get<Velocity>("raft_base_jerk") })
    , raft_interface_config(
          ...
          Ratio(1.0),
          SpeedDerivatives{ raft_interface_train.settings.get<Velocity>("raft_interface_speed"),
                            raft_interface_train.settings.get<Acceleration>("raft_interface_acceleration"),
                            raft_interface_train.settings.get<Velocity>("raft_interface_jerk") })
    , raft_surface_config(
          PrintFeatureType::SupportInterface,
          ...
          Ratio(1.0),
          SpeedDerivatives{ raft_surface_train.settings.get<Velocity>("raft_surface_speed"),
                            raft_surface_train.settings.get<Acceleration>("raft_surface_acceleration"),
                            raft_surface_train.settings.get<Velocity>("raft_surface_jerk") })
    , support_roof_config(
          PrintFeatureType::SupportInterface,
          ...
          SpeedDerivatives{ support_roof_train.settings.get<Velocity>("speed_support_roof"),
    ...
    , support_bottom_config(
          ...
          SpeedDerivatives{ support_bottom_train.settings.get<Velocity>("speed_support_bottom"),
                            ...
                            support_bottom_train.settings.get<Velocity>("jerk_support_bottom") })
{
    ...
    for (size_t extruder_nr = 0; extruder_nr < extruder_count; extruder_nr++)
    {
        ...
        skirt_brim_config_per_extruder.emplace_back(
            ...
            SpeedDerivatives{ train.settings.get<Velocity>("skirt_brim_speed"),
        ...
        prime_tower_config_per_extruder.emplace_back(
            ...
            SpeedDerivatives{ train.settings.get<Velocity>("speed_prime_tower"),
...
void cura::PathConfigStorage::handleInitialLayerSpeedup(const SliceDataStorage& storage, const LayerIndex& layer_nr, const size_t initial_speedup_layer_count)
{
    ...
    {
        global_first_layer_config_per_extruder.emplace_back(SpeedDerivatives{ extruder.settings.get<Velocity>("speed_print_layer_0"),
    ...
    { // extruder configs: travel, skirt/brim (= shield)
        ...
        {
            const ExtruderTrain& train = Application::getInstance().current_slice->scene.extruders[extruder_nr];
            SpeedDerivatives initial_layer_travel_speed_config{ train.settings.get<Velocity>("speed_travel_layer_0"),
    ...
    { // meshes
        for (size_t mesh_idx = 0; mesh_idx < storage.meshes.size(); mesh_idx++)
        {
            ...
            SpeedDerivatives initial_layer_speed_config{ mesh.settings.get<Velocity>("speed_print_layer_0"),
