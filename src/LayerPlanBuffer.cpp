...
void LayerPlanBuffer::insertPreheatCommand(ExtruderPlan& extruder_plan_before, const Duration time_before_extruder_plan_end, const size_t extruder_nr, const Temperature temp)
{
    ...
    for (unsigned int path_idx = extruder_plan_before.paths.size() - 1; int(path_idx) != -1; path_idx--)
    {
        ...
        if (acc_time >= time_before_extruder_plan_end)
        {
            ...
            extruder_plan_before.insertCommand(NozzleTempInsert{ .path_idx = path_idx, .extruder = extruder_nr, .temperature = temp, .wait = wait, .time_after_path_start = acc_time - time_before_extruder_plan_end });
            return;
        }
    }
    constexpr bool wait = false;
    constexpr size_t path_idx = 0;
    extruder_plan_before.insertCommand(NozzleTempInsert{ .path_idx = path_idx, .extruder = extruder_nr, .temperature = temp, .wait = wait }); // insert at start of extruder plan if time_after_extruder_plan_start > extruder_plan.time
}
...
void LayerPlanBuffer::insertPreheatCommand_multiExtrusion(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx)
{
    ...
    extruder_plans[0]->insertCommand(NozzleTempInsert{ .path_idx = path_idx, .extruder = extruder, .temperature = initial_print_temp, .wait = wait }); // insert preheat command at verfy beginning of buffer
}
...
void LayerPlanBuffer::insertFinalPrintTempCommand(std::vector<ExtruderPlan*>& extruder_plans, unsigned int last_extruder_plan_idx)
{
    ...
    { // insert temp command in precool_extruder_plan
        ...
        precool_extruder_plan->insertCommand(NozzleTempInsert{ .path_idx = path_idx, .extruder = extruder, .temperature = final_print_temp, .wait = wait, .time_after_path_start = time_after_path_start });
    }
}
