...
size_t LightningGenerator::WallLineCount::getCountAt(const size_t layer_nr) const
{
    ...
    return std::get<const std::vector<SliceLayer>*>(*this)->at(layer_nr).extra_wall_count_infill;
}
...
LightningGenerator::LightningGenerator(const SliceMeshStorage& mesh)
{
    ...
    generate(layer_thickness, infill_line_width, &mesh.layers, line_distance, overhang_angle, prune_angle, straightening_angle, areas_per_layer);
}
...
LightningGenerator::LightningGenerator(const SupportStorage& support)
{
    ...
    const auto infill_wall_line_count = settings.get<size_t>("support_wall_count");
...
void LightningGenerator::generateInitialInternalOverhangs(const coord_t infill_line_width, const WallLineCount& infill_wall_line_count, const std::vector<Shape>& shape_per_layer)
{
    ...
    {
        const coord_t infill_wall_offset = -static_cast<coord_t>(infill_wall_line_count.getCountAt(layer_nr)) * infill_line_width;
...
void LightningGenerator::generateTrees(const coord_t infill_line_width, const WallLineCount& infill_wall_line_count, const std::vector<Shape>& shape_per_layer)
{
