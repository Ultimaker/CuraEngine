    ...
    , current_overhang(current_overhang)
{
    ...
    unsupported_points.sort(
        [&radius](const UnsupCell& a, const UnsupCell& b)
        {
            ...
            return std::abs(b.dist_to_boundary - a.dist_to_boundary) > radius ? a.dist_to_boundary < b.dist_to_boundary : (std::hash<Point>{}(a.loc) % prime_for_hash) < (std::hash<Point>{}(b.loc) % prime_for_hash);
        });
