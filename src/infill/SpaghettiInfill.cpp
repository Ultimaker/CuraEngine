...
void SpaghettiInfill::generateSpaghettiInfill(SliceMeshStorage& mesh)
{
    ...
    int spaghetti_max_layer_count = std::max(1, static_cast<int>(mesh.getSettingInMicrons("spaghetti_max_height") / iconic_layer_height));
    ...
    {
        const double layer_height_mm = (layer_idx == 0)? mesh.getSettingInMillimeters("layer_height_0") : mesh.getSettingInMillimeters("layer_height"); // changes after the first layer
        ...
        for (SliceLayerPart& slice_layer_part : layer.parts)
        {
            ...
            {
                SpaghettiInfill::InfillPillar& pillar = addPartToPillarBase(infill_part, pillar_base, connection_inset_dist, layer_height_mm);
...
SpaghettiInfill::InfillPillar& SpaghettiInfill::addPartToPillarBase(const PolygonsPart& infill_part, std::list<SpaghettiInfill::InfillPillar>& pillar_base, coord_t connection_inset_dist, double layer_height_mm)
{
    ...
    for (auto it = pillar_base.begin(); it != pillar_base.end(); ++it)
    {
        ...
        {
            pillar.total_volume_mm3 += INT2MM(INT2MM(infill_part.area())) * layer_height_mm;
    ...
    { // couldn't connect to any existing pillar
        pillar_base.emplace_back(infill_part, connection_inset_dist, layer_height_mm);
