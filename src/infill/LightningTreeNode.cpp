...
void LightningTreeNode::propagateToNextLayer(std::vector<LightningTreeNodeSPtr>& next_trees,
...
LightningTreeNode::LightningTreeNode(const Point& p, const std::optional<Point>& last_grounding_location /*= std::nullopt*/) : is_root(true), p(p), last_grounding_location(last_grounding_location)
{
}
...
bool LightningTreeNode::realign(const Polygons& outlines, const LocToLineGrid& outline_locator, std::vector<LightningTreeNodeSPtr>& rerooted_parts)
{
    ...
    if (outlines.inside(p, true))
    {
        ...
        const auto remove_unconnected_func{ [&](const LightningTreeNodeSPtr& child)
                                            {
...
LightningTreeNode::RectilinearJunction LightningTreeNode::straighten(const coord_t magnitude, const Point& junction_above, const coord_t accumulated_dist, const coord_t max_remove_colinear_dist2)
{
    ...
    if (children.size() == 1)
    {
        ...
        { // remove nodes on linear segments
            ...
            if (parent_node && vSize2(child_p->p - parent_node->p) < max_remove_colinear_dist2 && LinearAlg2D::getDist2FromLineSegment(parent_node->p, p, child_p->p) < close_enough)
            {
