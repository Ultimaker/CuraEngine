...
void TreeSupport::generateSupportAreas(SliceDataStorage& storage)
{
    ...
    for (auto [counter, processing] : grouped_meshes | ranges::views::enumerate)
    {
        ...
        cura::parallel_for<coord_t>(LayerIndex(0),
...
void TreeSupport::mergeHelper(std::map<TreeSupportElement, AABB>& reduced_aabb,
        ...
        for (const auto& reduced_check : reduced_aabb)
        {
            ...
            if (aabb.hit(influence_aabb))
            {
                ...
                if (merging_min_and_regular_xy || merging_gracious_and_non_gracious || increased_to_model_radius > config.max_to_model_radius_increase
                    || (! merging_to_bp && larger_to_model_dtt < config.min_dtt_to_model && ! reduced_check.first.supports_roof && ! influence.first.supports_roof) || merging_different_range_limits)
                {
                    continue;
                }
                ...
                if (merging_to_bp)
                {
                    ...
                    relevant_redu = insert_bp_areas.count(reduced_check.first) ? insert_bp_areas[reduced_check.first] : (to_bp_areas.count(reduced_check.first) ? to_bp_areas.at(reduced_check.first) : Polygons());
                }
                else
                {
                    relevant_infl = to_model_areas.count(influence.first) ? to_model_areas.at(influence.first) : Polygons();
                    relevant_redu = insert_model_areas.count(reduced_check.first) ? insert_model_areas[reduced_check.first] : (to_model_areas.count(reduced_check.first) ? to_model_areas.at(reduced_check.first) : Polygons());
                }

                const bool red_bigger = config.getCollisionRadius(reduced_check.first) > config.getCollisionRadius(influence.first);
                std::pair<TreeSupportElement, Polygons> smaller_rad = red_bigger ? std::pair<TreeSupportElement, Polygons>(influence.first, relevant_infl) : std::pair<TreeSupportElement, Polygons>(reduced_check.first, relevant_redu);
                std::pair<TreeSupportElement, Polygons> bigger_rad = red_bigger ? std::pair<TreeSupportElement, Polygons>(reduced_check.first, relevant_redu) : std::pair<TreeSupportElement, Polygons>(influence.first, relevant_infl);
                ...
                const Polygons small_rad_increased_by_big_minus_small = TreeSupportUtils::safeOffsetInc(smaller_rad.second,
                                                                                                        real_radius_delta,
                ...
                if (intersect.area() > 1) // dont use empty as a line is not empty, but for this use-case it very well may be (and would be one layer down as union does not keep lines)
                {
                    ...
                    const TreeSupportElement key(
                        reduced_check.first, influence.first, layer_idx - 1, new_pos, increased_to_model_radius, getRadiusFunction, config.diameter_scale_bp_radius, config.branch_radius, config.diameter_angle_scale_factor);
                    ...
                    const auto getIntersectInfluence = [&](const PropertyAreasUnordered& insert_infl, const PropertyAreas& infl_areas)
                    {
                        ...
                        const Polygons small_rad_increased_by_big_minus_small_infl = TreeSupportUtils::safeOffsetInc(infl_small,
                                                                                                                     real_radius_delta,
                                                                                                                     volumes_.getCollision(smaller_collision_radius, layer_idx - 1, use_min_radius),
    ...
    while (buckets_area.size() > 1)
    {
        ...
        cura::parallel_for<size_t>(0,
                                   ...
                                   [&](size_t bucket_pair_idx)
                                   {
                                       ...
                                       mergeHelper(buckets_aabb[bucket_pair_idx],
...
std::optional<TreeSupportElement> TreeSupport::increaseSingleArea(AreaIncreaseSettings settings,
                                                                  ...
                                                                  const bool mergelayer)
{
    ...
    if (settings.move)
    {
        ...
        if (overspeed > 0)
        {
            ...
            increased = TreeSupportUtils::safeOffsetInc(increased,
                                                        overspeed,
    ...
    if (settings.increase_radius && check_layer_data.area() > 1)
    {
        ...
        // If the support_rest_preference is GRACEFUL, increase buildplate_radius_increases anyway. This does ONLY affect the CollisionRadius, as the regular radius only includes the buildplate_radius_increases when the SupportElement is
        // to_buildplate (which it can not be when support_rest_preference is GRACEFUL). If the branch later rests on the buildplate the to_buildplate flag will only need to be updated to ensure that the radius is also correctly increased.
        ...
        // A better way could be changing avoidance to model to not include the buildplate and then calculate avoidances by combining the to model avoidance without the radius increase with the to buildplate avoidance with the larger
        // radius. This would require ensuring all requests for the avoidance would have to ensure that the correct hybrid avoidance is requested (which would only be relevant when support_rest_preference is GRACEFUL) Also unioning areas
        // when an avoidance is requested may also have a relevant performance impact, so there can be an argument made that the current workaround is preferable.
    ...
    if (current_elem.influence_area_limit_active && ! current_elem.use_min_xy_dist && check_layer_data.area() > 1 && (current_elem.to_model_gracious || current_elem.distance_to_top <= config.min_dtt_to_model))
    {
        const coord_t max_radius_increase =
            std::max(static_cast<coord_t>((config.branch_radius - config.min_radius) / config.tip_layers),
...
void TreeSupport::increaseAreas(PropertyAreasUnordered& to_bp_areas,
                                ...
                                const bool mergelayer)
{
    ...
    cura::parallel_for<size_t>(
        ...
        [&](const size_t idx)
        {
            ...
            // When z distance is more than one layer up and down the Collision used to calculate the wall restriction will always include the wall (and not just the xy_min_distance) of the layer above and below like this (d = blocked area
            // because of z distance):
            /*
            ...
            insertSetting(AreaIncreaseSettings(AvoidanceType::FAST, fast_speed, ! increase_radius, ! no_error, elem.use_min_xy_dist, move),
                          true); // simplifying is very important for performance, but before an error is compensated by moving faster it makes sense to check to see if the simplifying has caused issues
            ...
            // Proper way would be to offset getAccumulatedPlaceable0 by -radius first, but the small benefit to maybe detect an error, that should not be happening anyway is not worth the performance impact in the expected case when a
            // branch rests on the model.
            ...
            for (AreaIncreaseSettings settings : order)
            {
                if (settings.move)
                {
                    ...
                    {
                        offset_slow = TreeSupportUtils::safeOffsetInc(*parent->area,
                    ...
                    if ((settings.increase_speed != slow_speed) && offset_fast.empty())
                    {
                        ...
                        {
                            offset_fast = TreeSupportUtils::safeOffsetInc(*parent->area,
                        ...
                        else
                        {
                            ...
                            offset_fast = TreeSupportUtils::safeOffsetInc(
                                              offset_slow, delta_slow_fast, wall_restriction, safe_movement_distance, safe_movement_distance + radius, offset_independent_faster ? 2 : 1, config.support_line_distance / 2, &config.simplifier)
                                              .unionPolygons();
                        }
                    }
                }
                ...
                if (! settings.no_error)
                {
                    ...
                    {
                        spdlog::warn("Influence area could not be increased! Data about the Influence area: "
                ...
                else
                {
                    result = increaseSingleArea(
                        settings, layer_idx, parent, settings.increase_speed == slow_speed ? offset_slow : offset_fast, to_bp_data, to_model_data, inc_wo_collision, std::max(settings.increase_speed - fast_speed, coord_t(0)), mergelayer);
                }
    ...
    else // can not add graceful => just place it here and hope for the best
    {
        ...
        if (! valid_place_area.inside(best, true))
        {
            ...
            else
            {
                ...
                for (coord_t radius_offset :
                     { -config.getCollisionRadius(first_elem), -config.getCollisionRadius(first_elem) / 2, coord_t(0) }) // Interestingly the first radius is working most of the time, even though it seems like it shouldn't.
                {
...
{
    // Initialize points on layer 0, with a "random" point in the influence area. Point is chosen based on an inaccurate estimate where the branches will split into two, but every point inside the influence area would produce a valid
    // result.
    ...
    for (TreeSupportElement* init : move_bounds[0])
    {
        ...
        if (config.support_rest_preference != RestPreference::BUILDPLATE)
        {
            ...
            {
                // If the support_rest_preference is GRACEFUL the collision radius is increased, but the radius will only be increased if the element is to_buildplate, so if the branch rests on the buildplate, the element will have to be
        ...
        for (TreeSupportElement* elem : move_bounds[layer_idx])
        {
            ...
            if (elem->result_on_layer == Point(-1, -1)) // Check if the resulting center point is not yet set.
            {
                if (elem->to_buildplate || (! elem->to_buildplate && elem->distance_to_top < config.min_dtt_to_model && ! elem->supports_roof))
                {
                    ...
                    {
                        // When the roof was not able to generate downwards enough, the top elements may have not moved, and have result_on_layer already set. As this branch needs to be removed => all parents result_on_layer have to be
                        // invalidated.
                        parent->result_on_layer = Point(-1, -1);
                    }
                    continue;
...
void TreeSupport::generateBranchAreas(std::vector<std::pair<LayerIndex, TreeSupportElement*>>& linear_data,
                                       ...
                                       for (TreeSupportElement* parent : data_pair.first->parents)
                                       {
                                           if (config.getRadius(*parent) != config.getCollisionRadius(*parent))
                                           {
                                               ...
                                               max_outer_wall_distance =
                                                   std::max(max_outer_wall_distance, vSize(data_pair.first->result_on_layer - parent->result_on_layer) - (config.getRadius(*data_pair.first) - config.getRadius(*parent)));
                                           }
                                       }
...
void TreeSupport::dropNonGraciousAreas(std::vector<std::unordered_map<TreeSupportElement*, Polygons>>& layer_tree_polygons,
    ...
    cura::parallel_for<size_t>(0,
                               ...
                               [&](const size_t idx)
                               {
                                   ...
                                   bool non_gracious_model_contact =
                                       ! elem->to_model_gracious && ! inverse_tree_order.count(elem); // If an element has no child, it connects to whatever is below as no support further down for it will exist.
...
void TreeSupport::filterFloatingLines(std::vector<Polygons>& support_layer_storage)
{
    ...
    cura::parallel_for<coord_t>(
        ...
        [&](const LayerIndex layer_idx)
        {
            support_layer_storage[layer_idx] =
                config.simplifier.polygon(PolygonUtils::unionManySmall(support_layer_storage[layer_idx].smooth(FUDGE_LENGTH))).offset(-open_close_distance).offset(open_close_distance * 2).offset(-open_close_distance);
    ...
    cura::parallel_for<coord_t>(1,
                                ...
                                [&](const LayerIndex layer_idx)
                                {
                                    ...
                                    Polygons outer_walls = TreeSupportUtils::toPolylines(support_layer_storage[layer_idx - 1].getOutsidePolygons())
    ...
    // Check if holes are so close to each other that two lines will be printed directly next to each other, which is assumed stable (as otherwise the simulated support pattern will not work correctly) and remove all remaining, invalid
    // holes
    cura::parallel_for<coord_t>(1,
                                support_layer_storage.size(),
                                [&](const LayerIndex layer_idx)
    ...
    spdlog::debug(
        "Time to union areas: {} ms Time to evaluate which hole rest on which other hole: {} ms Time to see which holes are not resting on anything valid: {} ms remove all holes that are invalid and not close enough to a valid hole: {} ms",
        dur_union,
    ...
    cura::parallel_for<coord_t>(
        ...
        [&](const LayerIndex layer_idx)
        {
            ...
            if (! storage.support.supportLayers[layer_idx].support_roof.empty() && support_layer_storage[layer_idx].intersection(storage.support.supportLayers[layer_idx].support_roof).area() > 1)
            {
                ...
                {
                    Polygons tree_lines;
                    tree_lines = tree_lines.unionPolygons(TreeSupportUtils::generateSupportInfillLines(support_layer_storage[layer_idx], config, false, layer_idx, config.support_line_distance, storage.support.cross_fill_provider, true)
...
void TreeSupport::drawAreas(std::vector<std::set<TreeSupportElement*>>& move_bounds, SliceDataStorage& storage)
{
    ...
    spdlog::info("Time used for drawing subfuctions: generateBranchAreas: {} ms smoothBranchAreas: {} ms dropNonGraciousAreas: {} ms filterFloatingLines: {} ms finalizeInterfaceAndSupportAreas {} ms",
                 dur_gen_tips,
                 dur_smooth,
                 dur_drop,
                 dur_filter,
