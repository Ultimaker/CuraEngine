...
static int findAdjacentEnclosingPoly(const ConstPolygonRef& enclosed_inset, const std::vector<ConstPolygonPointer>& possible_enclosing_polys, const coord_t max_gap)
{
    ...
    for (unsigned int enclosing_poly_idx = 0; enclosing_poly_idx < possible_enclosing_polys.size(); ++enclosing_poly_idx)
    {
        ...
        // As holes don't overlap, if the insets intersect, it is safe to assume that the enclosed inset is inside
        // the enclosing inset
...
{
    // Bin the insets in order to print the inset indices together, and to optimize the order of each bin to
    // reduce travels.
    ...
    std::vector<std::vector<std::vector<ExtrusionJunction>>> insets(num_insets);
...
void InsetOrderOptimizer::moveInside()
{
    ...
    if (PolygonUtils::moveInside(part.insets[0], p, outer_wall_line_width * 1.1f) != NO_INDEX)
    {
        ...
        else
        {
            ...
            // This can occur when the last wall finished at a right angle corner as the first move just moved p along
            // one edge rather than into the part.
            if (PolygonUtils::moveInside(part.insets[0], p, outer_wall_line_width * 1.1f) != NO_INDEX)
            {
                // Move to p if it is not closer than a line width from the centre line of the outer wall
                if (retraction_region.inside(p))
...
void InsetOrderOptimizer::processHoleInsets()
{
    ...
    {
        // Part has no holes - the code below has a problem in that it causes the z-seams not to be aligned in the
        // specific situation where infill was being printed before walls, the inner walls were printed before the outer
        // and the part has no holes!
        return;
    }
    ...
    // If polys are closer than this, they are considered adjacent
    const coord_t max_gap = std::max(wall_line_width_0, wall_line_width_x) * 1.1f;
    ...
    if (!outer_inset_first && mesh.settings.get<bool>("infill_before_walls"))
    {
        ...
        // So detect any higher level insets that don't surround holes and output them before the insets that do
        // surround holes
        for (unsigned inset_level = num_insets - 1; inset_level > 0; --inset_level)
        {
            ...
            for (unsigned inset_idx = 0; inset_idx < inset_polys[0].size() && inset_idx < inset_polys[inset_level].size(); ++inset_idx)
            {
                ...
                {
                    // The inset didn't surround the level 0 inset with the same inset_idx but maybe it surrounds
                    // another hole.
    ...
    // If the z-seam location on the part's outline can be determined here, optimize to minimize the distance travelled
    // from the last hole to the z-seam - one possible benefit of this strategy is that by minimizing the distance
    // travelled to the outline, the accuracy of that movement is potentially improved (less overshoot, backlash, etc.)
    // which could make a visible difference (especially if the outer wall is printed first).

    // If we can't determine here where the z-seam on the outline will be, optimize to minimize the distance travelled
    // from the current location to the first hole
    ...
    // This will consume all of the insets that surround holes but not the insets next to the outermost wall of the
    // model
    for (size_t outer_poly_order_idx = 0; outer_poly_order_idx < order_optimizer.paths.size(); ++outer_poly_order_idx)
    {
        ...
        // The indices of the walls that touch the hole's outer wall
        std::vector<unsigned int> hole_level_1_wall_indices;
        ...
        if (inset_polys.size() > 1)
        {
            ...
            {
                // Now test for the case where we are printing the outer walls first and this level 1 inset also touches
                // other outer walls.
                // In this situation we don't want to output the level 1 inset until after all the outer walls (that it
                // touches) have been printed
                if (outer_inset_first)
                {
                    ...
                    // Number of outlines that have yet to be output that are touched by this level 1 inset
                    int num_future_outlines_touched = 0;
            ...
            {
                // We didn't find a level 1 inset that encloses this hole so now look to see if there is one or more
                // level 1 insets that simply touch this hole and use those instead - however, as the level 1 insets
                // will also touch other holes and/or the outer wall we don't want to do this when printing the outer
                // walls first
                    ...
                    for (unsigned int level_n_wall_idx = 0; level_n_wall_idx < inset_polys[inset_level].size(); ++level_n_wall_idx)
                    {
                        for (ConstPolygonRef enclosing_inset : enclosing_insets)
                        {
                            ...
                            if (PolygonUtils::polygonOutlinesAdjacent(level_n_inset, enclosing_inset, wall_line_width_x * 1.1f))
                            {
                                ...
                                // We've shortened the vector so decrement the index otherwise, we'll skip an element
                                --level_n_wall_idx;
                                break;
                            }
                        }
