...
InsetOrderOptimizer::value_type InsetOrderOptimizer::getRegionOrder(const auto& input, const bool outer_to_inner)
{
    ...
    rg::sort(
        locator_view, [](const auto& lhs, const auto& rhs) { return std::abs(lhs) < std::abs(rhs); }, &LineLoc::area);
    ...
    for (const LineLoc* root : roots)
    {
        ...
        {
            const std::function<void(const LineLoc*, const unsigned int)> initialize_nodes = [graph, root, &hole_roots, &min_node, &min_depth](const auto current_node, const auto depth)
            {
        ...
        // the depth is closest to root $r$
        {
            ...
            const std::function<void(const LineLoc*, const LineLoc*)> set_order_constraints = [&order, &min_node, &root_, graph, outer_to_inner](const auto& current_node, const auto& parent_node)
            {
