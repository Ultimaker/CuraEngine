...
TreeSupportTipGenerator::TreeSupportTipGenerator(const SliceDataStorage& storage, const SliceMeshStorage& mesh, TreeModelVolumes& volumes_s)
    ...
    , support_roof_layers(mesh.settings.get<bool>("support_roof_enable") ? round_divide(mesh.settings.get<coord_t>("support_roof_height"), config.layer_height)
    ...
    , support_roof_line_distance(use_fake_roof
                                     ? (config.support_pattern == EFillMethod::TRIANGLES ? 3 : (config.support_pattern == EFillMethod::GRID ? 2 : 1)) * (config.support_line_width * 100 / mesh.settings.get<double>("support_tree_top_rate"))
    ...
    , force_minimum_roof_area(use_fake_roof || SUPPORT_TREE_MINIMUM_ROOF_AREA_HARD_LIMIT)
{
    ...
    else
    {
        ...
        //     Could also be an user setting or differently calculated. Idea is that if an overhang does not turn valid in double the amount of layers a slope of support angle would take to travel xy_distance, nothing reasonable will come
        //     from it. The 2*z_distance_delta is only a catch for when the support angle is very high.
    }
...
std::vector<TreeSupportTipGenerator::LineInformation> TreeSupportTipGenerator::convertLinesToInternal(Polygons polylines, LayerIndex layer_idx)
{
    ...
    // NOTE2: When refactoring ensure that avoidance to buildplate is only requested when support_rest_preference == RestPreference::BUILDPLATE as otherwise it has not been precalculated (causing long delays while it is calculated when
    // requested here).
...
std::function<bool(std::pair<Point, TreeSupportTipGenerator::LineStatus>)> TreeSupportTipGenerator::getEvaluatePointForNextLayerFunction(size_t current_layer)
{
    std::function<bool(std::pair<Point, LineStatus>)> evaluatePoint = [=](std::pair<Point, LineStatus> p)
    {
        if (config.support_rest_preference != RestPreference::GRACEFUL
            && ! volumes_.getAvoidance(config.getRadius(0), current_layer - 1, p.second == LineStatus::TO_BP_SAFE ? AvoidanceType::FAST_SAFE : AvoidanceType::FAST, false, ! xy_overrides).inside(p.first, true))
        {
            return true;
        }
...
std::pair<std::vector<TreeSupportTipGenerator::LineInformation>, std::vector<TreeSupportTipGenerator::LineInformation>>
    TreeSupportTipGenerator::splitLines(std::vector<TreeSupportTipGenerator::LineInformation> lines, std::function<bool(std::pair<Point, TreeSupportTipGenerator::LineStatus>)> evaluatePoint)
{
...
void TreeSupportTipGenerator::dropOverhangAreas(const SliceMeshStorage& mesh, std::vector<Polygons>& result, bool roof)
{
    ...
    cura::parallel_for<coord_t>(
        ...
        [&](const LayerIndex layer_idx)
        {
            ...
            Polygons overhang_regular = TreeSupportUtils::safeOffsetInc(
                mesh.overhang_areas[layer_idx + z_distance_delta], roof ? roof_outset : support_outset, relevant_forbidden, config.min_radius * 1.75 + config.xy_min_distance, 0, 1, config.support_line_distance / 2, &config.simplifier);
            Polygons remaining_overhang = mesh.overhang_areas[layer_idx + z_distance_delta].offset(roof ? roof_outset : support_outset).difference(overhang_regular).intersection(relevant_forbidden).difference(model_outline);
    ...
    cura::parallel_for<coord_t>(0, result.size(), [&](const LayerIndex layer_idx) { result[layer_idx] = result[layer_idx].unionPolygons(); });
}
...
void TreeSupportTipGenerator::calculateRoofAreas(const cura::SliceMeshStorage& mesh)
{
    ...
    cura::parallel_for<coord_t>(
        ...
        [&](const LayerIndex layer_idx)
        {
            ...
            Polygons forbidden_here = volumes_.getAvoidance(config.getRadius(0), layer_idx, (only_gracious || ! config.support_rests_on_model) ? AvoidanceType::FAST : AvoidanceType::COLLISION, config.support_rests_on_model, ! xy_overrides)
                                          .offset(-config.getRadius(0), ClipperLib::jtRound);

            // todo Since arachnea the assumption that an area smaller then line_width is not printed is no longer true all such safeOffset should have config.support_line_width replaced with another setting. It should still work in most
            // cases, but it should be possible to create a situation where a overhang outset lags though a wall. I will take a look at this later.
            Polygons full_overhang_area = TreeSupportUtils::safeOffsetInc(
                mesh.full_overhang_areas[layer_idx + z_distance_delta].unionPolygons(dropped_overhangs[layer_idx]), roof_outset, forbidden_here, config.support_line_width, 0, 1, config.support_line_distance / 2, &config.simplifier);

            for (LayerIndex dtt_roof = 0; dtt_roof < support_roof_layers && layer_idx - dtt_roof >= 1; dtt_roof++)
            {
                const Polygons forbidden_next =
                    volumes_.getAvoidance(config.getRadius(0), layer_idx - (dtt_roof + 1), (only_gracious || ! config.support_rests_on_model) ? AvoidanceType::FAST : AvoidanceType::COLLISION, config.support_rests_on_model, ! xy_overrides)
                        .offset(-config.getRadius(0), ClipperLib::jtRound);
    ...
    cura::parallel_for<coord_t>(0,
                                ...
                                [&](const LayerIndex layer_idx)
                                {
                                    // Now, because the avoidance/collision was subtracted above, the overhang parts that are of xy distance were removed, so to merge areas that should have been one offset by xy_min_distance and then undo
                                    // it. In a perfect world the offset here would be of a mode that makes sure that area.offset(config.xy_min_distance).unionPolygons().offset(-config.xy_min_distance) = area if there is only one polygon in
                                    // said area. I have not encountered issues with using the default mitered here. Could be that i just have not encountered an issue with it yet though.
                                    potential_support_roofs[layer_idx] =
                                        potential_support_roofs[layer_idx].unionPolygons().offset(config.xy_min_distance).unionPolygons().offset(-config.xy_min_distance).unionPolygons(potential_support_roofs[layer_idx]);
                                });
    ...
    cura::parallel_for<coord_t>(0, additional_support_roofs.size(), [&](const LayerIndex layer_idx) { support_roof_drawn[layer_idx] = support_roof_drawn[layer_idx].unionPolygons(additional_support_roofs[layer_idx]); });
}


void TreeSupportTipGenerator::addPointAsInfluenceArea(std::vector<std::set<TreeSupportElement*>>& move_bounds,
    ...
    Polygons area = circle.offset(0);
    {
        ...
        if (! already_inserted[insert_layer].count(p.first / ((config.min_radius + 1) / 10)))
        {
            ...
            TreeSupportElement* elem =
                new TreeSupportElement(dtt, insert_layer, p.first, to_bp, gracious, ! xy_overrides, dont_move_until, roof, safe_radius, force_tip_to_roof, skip_ovalisation, support_tree_limit_branch_reach, support_tree_branch_reach_limit);
    ...
    for (LineInformation line : lines)
    {
        ...
        for (auto [idx, point_data] : line | ranges::views::enumerate)
        {
            ...
            addPointAsInfluenceArea(
                move_bounds, point_data, 0, insert_layer_idx - dtt_roof_tip, dont_move_until > dtt_roof_tip ? dont_move_until - dtt_roof_tip : 0, dtt_roof_tip != 0 || supports_roof, disable_ovalization, additional_ovalization_targets);
        }
    }
}
...
void TreeSupportTipGenerator::generateTips(SliceDataStorage& storage,
                                           ...
                                           std::vector<Polygons>& placed_support_lines_support_areas)
{
    ...
    cura::parallel_for<coord_t>(
        ...
        [&](const LayerIndex layer_idx)
        {
            ...
            //     This can NOT be done in the for loop as an area may NOT have a roof even if it is larger than the minimum_roof_area when it is only larger because of the support horizontal expansion and it would not have a roof if the
            //     overhang is offset by support roof horizontal expansion instead. (At least this is the current behavior of the regular support)
            ...
            Polygons overhang_regular = TreeSupportUtils::safeOffsetInc(core_overhang, support_outset, relevant_forbidden, config.min_radius * 1.75 + config.xy_min_distance, 0, 1, config.support_line_distance / 2, &config.simplifier);
            Polygons remaining_overhang = core_overhang.offset(support_outset).difference(overhang_regular.offset(config.support_line_width * 0.5)).intersection(relevant_forbidden);
            ...
            {
                coord_t offset_current_step = extra_total_offset_acc + 2 * config.support_line_width > config.min_radius ? std::min(config.support_line_width / 8, extra_outset - extra_total_offset_acc)
                ...
                Polygons overhang_offset = TreeSupportUtils::safeOffsetInc(
                    overhang_regular, 1.5 * extra_total_offset_acc, volumes_.getCollision(0, layer_idx, true), config.xy_min_distance + config.support_line_width, 0, 1, config.support_line_distance / 2, &config.simplifier);
                ...
                Polygons next_overhang = TreeSupportUtils::safeOffsetInc(
                    remaining_overhang, extra_total_offset_acc, volumes_.getCollision(0, layer_idx, true), config.xy_min_distance + config.support_line_width, 0, 1, config.support_line_distance / 2, &config.simplifier);
                ...
                for (Polygons& remaining_overhang_part : remaining_overhang.splitIntoParts(false))
                {
                    ...
                    for (size_t lag_ctr = 1; lag_ctr <= max_overhang_insert_lag && ! overhang_lines.empty() && layer_idx - coord_t(lag_ctr) >= 1; lag_ctr++)
                    {
                        ...
                        if (support_roof_layers)
                        {
                            ...
                            std::function<bool(std::pair<Point, LineStatus>)> evaluatePartOfRoof = [&](std::pair<Point, LineStatus> p) { return support_roof_drawn[layer_idx - lag_ctr].inside(p.first, true); };
            ...
            for (std::pair<Polygons, bool> overhang_pair : overhang_processing)
            {
                ...
                Polygons polylines = ensureMaximumDistancePolyline(generateLines(overhang_outset, roof_allowed_for_this_part, layer_idx + roof_allowed_for_this_part),
