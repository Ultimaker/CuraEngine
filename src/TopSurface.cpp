...
bool TopSurface::ironing(const SliceDataStorage& storage, const SliceMeshStorage& mesh, const GCodePathConfig& line_config, LayerPlan& layer, const FffGcodeWriter& gcode_writer) const
{
    ...
    Infill infill_generator(pattern,
                            zig_zaggify_infill,
                            connect_polygons,
                            ironed_areas,
                            line_width,
    ...
    if (! ironing_paths.empty())
    {
        ...
        InsetOrderOptimizer wall_orderer(
            gcode_writer, storage, layer, mesh.settings, extruder_nr, line_config, line_config, line_config, line_config, retract_before_outer_wall, wipe_dist, wipe_dist, extruder_nr, extruder_nr, z_seam_config, ironing_paths);
