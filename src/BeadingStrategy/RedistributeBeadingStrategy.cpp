...
namespace cura
{
    BeadingStrategy::Beading RedistributeBeadingStrategy::compute(coord_t thickness, coord_t bead_count) const
    {
        ...
        if (outer_bead_ratio != 1._r) // Only update if needed
        {
            // Update outer beads
            ...
            const coord_t total_inner_bead_width = std::accumulate(inner_bead_width_begin, inner_bead_width_end, static_cast<coord_t>(0));
            const coord_t total_required_inner_bead_width = thickness - ret.bead_widths.front() - ret.bead_widths.back();
            const Ratio inner_bead_ratio = static_cast<double>(total_required_inner_bead_width) / static_cast<double>(total_inner_bead_width);
            ...
            // TODO: insert inner_bead if needed

            // TODO: lock position of outer wall
