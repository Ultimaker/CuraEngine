...
void BeadingOrderOptimizer::optimize(std::vector<std::vector<ExtrusionLine>>& polygons_per_index, std::vector<std::vector<ExtrusionLine>>& polylines_per_index, bool reduce_overlapping_segments, bool connect_odd_lines_to_polygons)
{
...
BeadingOrderOptimizer::BeadingOrderOptimizer(std::vector<std::vector<ExtrusionLine>>& polylines_per_index)
: polylines_per_index(polylines_per_index)
{
    for (auto& polylines : polylines_per_index)
    {
        for (std::vector<ExtrusionLine>::iterator polyline_it = polylines.begin(); polyline_it != polylines.end(); ++polyline_it)
        {
...
void BeadingOrderOptimizer::fuzzyConnect(std::vector<std::vector<ExtrusionLine>>& polygons_per_index, coord_t snap_dist, bool reduce_overlapping_segments, bool connect_odd_lines_to_polygons)
{
    struct Locator
    {
    ...
    for (std::vector<ExtrusionLine>& polys : polylines_per_index)
    {
    ...
    for (std::vector<ExtrusionLine>& polys : polylines_per_index)
    {
        for (std::vector<ExtrusionLine>::const_iterator poly_it = polys.begin(); poly_it != polys.end();)
        {
