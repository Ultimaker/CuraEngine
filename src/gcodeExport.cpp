...
/*
* Clips points for the Inner and Outer walls based on the pointsClipDistance setting.
* If a clip occurs and after GCode move, it is done a:
* - Retraction in Inner walls
* - Fast move in Outer walls
*
* This helps preventing blobs in the models surface
*/
			if(strcmp(path->config->name,"WALL-OUTER") == 0 || strcmp(path->config->name,"WALL-INNER") == 0)
			{
				double currentDistance = 0.0;
				//double targetDistance = path->config->lineWidth;
				double targetDistance = path->config->clip;
//				gcode.writeComment("Clip Value:  %d", path->config->clip);
						//440.0;
				...
				for(unsigned int p=path->points.size()-1; p>0; p--)
				{
					...
					if(currentDistance > targetDistance)
					{
						...
						*/
						...
						/**
						* Clip points to the last one valid
						*/
					...
					{
						/**
						* Pops points if distance is equal to current clip distance
						*/
					...
					{
						/*
						* It is possible to reduce the distance even more.
						* Save target clip distance
						* Pop last point by being invalid
						*/
