...
WallToolPaths::WallToolPaths(const Polygons& outline, const coord_t nominal_bead_width, const size_t inset_count, const coord_t wall_0_inset, const Settings& settings, const int layer_idx, SectionType section_type)
    : outline(outline)
...
WallToolPaths::WallToolPaths(const Polygons& outline, const coord_t bead_width_0, const coord_t bead_width_x, const size_t inset_count, const coord_t wall_0_inset, const Settings& settings, const int layer_idx, SectionType section_type)
    : outline(outline)
...
const std::vector<VariableWidthLines>& WallToolPaths::generate()
{
    ...
    const auto beading_strat = BeadingStrategyFactory::makeStrategy(bead_width_0,
                                                                    bead_width_x,
    ...
    SkeletalTrapezoidation wall_maker(prepared_outline, *beading_strat, beading_strat->getTransitioningAngle(), discretization_step_size, transition_filter_dist, allowed_filter_deviation, wall_transition_length, layer_idx, section_type);
    wall_maker.generateToolpaths(toolpaths);
    scripta::log("toolpaths_0",
                 toolpaths,
                 section_type,
                 layer_idx,
    ...
    scripta::log("toolpaths_4",
                 ...
                 scripta::PointVDI{ "width", &ExtrusionJunction::w },
                 scripta::PointVDI{ "perimeter_index", &ExtrusionJunction::perimeter_index });
    assert(std::is_sorted(toolpaths.cbegin(), toolpaths.cend(), [](const VariableWidthLines& l, const VariableWidthLines& r) { return l.front().inset_idx < r.front().inset_idx; })
           && "WallToolPaths should be sorted from the outer 0th to inner_walls");
    toolpaths_generated = true;
    scripta::log("toolpaths_5",
                 toolpaths,
                 section_type,
                 layer_idx,
                 scripta::CellVDI{ "is_closed", &ExtrusionLine::is_closed },
...
void WallToolPaths::simplifyToolPaths(std::vector<VariableWidthLines>& toolpaths, const Settings& settings)
{
    ...
    for (auto& toolpath : toolpaths)
    {
        toolpath = toolpath
                 ...
                 | ranges::views::filter([](const auto& line) { return ! line.empty(); }) | ranges::to_vector;
    }
}
...
{
    toolpaths.erase(std::remove_if(toolpaths.begin(), toolpaths.end(), [](const VariableWidthLines& lines) { return lines.empty(); }), toolpaths.end());
