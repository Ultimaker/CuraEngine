...
GCodePath* LayerPlan::getLatestPathWithConfig(const GCodePathConfig& config, SpaceFillType space_fill_type, const Ratio flow, bool spiralize, const Ratio speed_factor)
{
    ...
    if (paths.size() > 0 && paths.back().config == &config && !paths.back().done && paths.back().flow == flow && paths.back().speed_factor == speed_factor && paths.back().mesh == current_mesh) // spiralize can only change when a travel path is in between
    {
        return &paths.back();
    }
...
GCodePath& LayerPlan::addTravel(const Point p, const bool force_retract)
{
    ...
    if (comb != nullptr && !bypass_combing)
    {
        ...
        combed = comb->calc(perform_z_hops, perform_z_hops_only_when_collides, *last_planned_position, p, combPaths, was_inside, is_inside, max_distance_ignored, unretract_before_last_travel_move);
        if (combed)
        {
            ...
            { // check whether we want to retract
                ...
                if (combPaths.size() == 1)
                {
                    ...
                    if (mesh_or_extruder_settings.get<bool>("limit_support_retractions") &&
