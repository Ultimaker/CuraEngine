...
GCodePath& LayerPlan::addTravel(const Point2LL& p, const bool force_retract, const coord_t z_offset)
{
    ...
    if (! combed && ! is_first_travel_of_layer && last_planned_position_ && ! shorterThen(last_planned_position_.value().toPoint2LL() - p, retraction_config.retraction_min_travel_distance))
    {
...
GCodePath& LayerPlan::addTravel_simple(const Point2LL& p, GCodePath* path)
{
    ...
    last_planned_position_ = Point3LL(p.X, p.Y, 0);
    return *path;
}
