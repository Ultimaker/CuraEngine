...
void FffGcodeWriter::processRaft(const SliceDataStorage& storage)
{
    ...
    { // raft base layer
        ...
        for (const Polygons& raft_outline_path : raft_outline_paths)
        {
            ...
            infill_comp.generate(raft_paths, raft_polygons, raftLines, base_settings, layer_nr, debug::SectionType::ADHESION);
            if (! raft_paths.empty())
            {
    ...
    { // raft interface layer
        ...
        for (const Polygons& raft_outline_path : raft_outline_paths)
        {
            ...
            infill_comp.generate(raft_paths, raft_polygons, raft_lines, interface_settings, layer_nr, debug::SectionType::ADHESION);
    ...
    { // raft surface layers
        ...
        for (const Polygons& raft_outline_path : raft_outline_paths)
        {
            Infill infill_comp(EFillMethod::ZIG_ZAG,
                               ...
                               pocket_size);
            std::vector<VariableWidthLines> raft_paths; // Should remain empty, since we have no walls.
            infill_comp.generate(raft_paths, raft_polygons, raft_lines, surface_settings, layer_nr, debug::SectionType::ADHESION);
    ...
    const
{
    ...
    for (unsigned int combine_idx = 1; combine_idx < part.infill_area_per_combine_per_density[0].size(); combine_idx++)
    {
        ...
        { // combine different density infill areas (for gradual infill)
            ...
            infill_comp.generate(infill_paths, infill_polygons, infill_lines, mesh.settings, gcode_layer.getLayerNr(), debug::SectionType::INFILL, mesh.cross_fill_provider, lightning_layer, &mesh);
        }
    ...
    for (size_t density_idx = last_idx; static_cast<int>(density_idx) >= 0; density_idx--)
    {
        ...
        if (hasSkinEdgeSupport)
        {
            ...
            Infill infill_comp(pattern,
                               ...
                               zag_skip_count,
                               pocket_size);
            infill_comp.generate(wall_tool_paths.back(), infill_polygons, infill_lines, mesh.settings, gcode_layer.getLayerNr(), debug::SectionType::INFILL, mesh.cross_fill_provider, lightning_layer, &mesh);
                                             ...
                                             double fan_speed) const
{
    ...
    Infill infill_comp(pattern,
                       ...
                       zag_skip_count,
                       pocket_size);
    infill_comp.generate(skin_paths, skin_polygons, skin_lines, mesh.settings, gcode_layer.getLayerNr(), debug::SectionType::SKIN);
        ...
        for (unsigned int combine_idx = 0; combine_idx < part.infill_area_per_combine_per_density[0].size(); ++combine_idx)
        {
            ...
            for (size_t density_idx = max_density_idx; (density_idx + 1) > 0; --density_idx)
            {
                ...
                Infill infill_comp(support_pattern,
                                   ...
                                   zag_skip_count,
                                   pocket_size);
                infill_comp.generate(wall_toolpaths_here, support_polygons, support_lines, infill_extruder.settings, gcode_layer.getLayerNr(), debug::SectionType::SUPPORT, storage.support.cross_fill_provider);
            }
...
bool FffGcodeWriter::addSupportRoofsToGCode(const SliceDataStorage& storage, LayerPlan& gcode_layer) const
{
    ...
    Polygons roof_lines;
    roof_computation.generate(roof_paths, roof_polygons, roof_lines, roof_extruder.settings, gcode_layer.getLayerNr(), debug::SectionType::SUPPORT);
...
bool FffGcodeWriter::addSupportBottomsToGCode(const SliceDataStorage& storage, LayerPlan& gcode_layer) const
{
    ...
    bottom_computation.generate(bottom_paths, bottom_polygons, bottom_lines, bottom_extruder.settings, gcode_layer.getLayerNr(), debug::SectionType::SUPPORT);
