...
void FffGcodeWriter::addMeshPartToGCode(const SliceDataStorage& storage, const SliceMeshStorage* mesh, const int extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part, LayerPlan& gcode_layer, int layer_nr) const
{
    ...
    added_something |= processSanding(mesh, part, gcode_layer);
...
bool FffGcodeWriter::processSanding(const SettingsBaseVirtual* settings, const SliceLayerPart& part, LayerPlan& gcode_layer) const
{
    bool added_something = false;
    if (part.top_surface)
    {
        added_something |= part.top_surface->sand(settings, gcode_layer);
    }
    return added_something;
}
