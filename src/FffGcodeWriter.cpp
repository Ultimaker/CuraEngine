    ...
    const
{
    ...
    for (unsigned int combine_idx = 1; combine_idx < part.infill_area_per_combine_per_density[0].size(); combine_idx++)
    {
        ...
        { // combine different density infill areas (for gradual infill)
            ...
            infill_comp.generate(infill_paths, infill_polygons, infill_lines, mesh.settings, gcode_layer.getLayerNr(), SectionType::INFILL, mesh.cross_fill_provider, lightning_layer, &mesh);
        }
    ...
    for (size_t density_idx = last_idx; static_cast<int>(density_idx) >= 0; density_idx--)
    {
        ...
        if (hasSkinEdgeSupport)
        {
            ...
            Infill infill_comp(pattern,
                               ...
                               zag_skip_count,
                               pocket_size);
            infill_comp.generate(wall_tool_paths.back(), infill_polygons, infill_lines, mesh.settings, gcode_layer.getLayerNr(), SectionType::INFILL, mesh.cross_fill_provider, lightning_layer, &mesh);
        ...
        for (unsigned int combine_idx = 0; combine_idx < part.infill_area_per_combine_per_density[0].size(); ++combine_idx)
        {
            ...
            for (size_t density_idx = max_density_idx; (density_idx + 1) > 0; --density_idx)
            {
                ...
                int support_line_distance_here =
                    (part.custom_line_distance > 0 ? part.custom_line_distance : default_support_line_distance * density_factor); // the highest density infill combines with the next to create a grid with density_factor 1
