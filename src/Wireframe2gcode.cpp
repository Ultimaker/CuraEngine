...
void Wireframe2gcode::writeGCode()
{
    ...
    for (PolygonRef bottom_part : wireFrame.bottom_infill.roof_outlines)
    {
        ...
        {
            gcode.writeExtrusion(segment_to, speedBottom, extrusion_mm3_per_mm_flat, 0, PrintFeatureType::Skin);
        }
    }
    ...
    writeFill(wireFrame.bottom_infill.roof_insets, empty_outlines,
              [this](Wireframe2gcode&, WeaveConnectionPart& part, unsigned int segment_idx) {
                    ...
                    {
                        gcode.writeExtrusion(segment.to, speedBottom, extrusion_mm3_per_mm_connection, 0, PrintFeatureType::Skin);
                    }
                }
            ,
              [this](Wireframe2gcode&, WeaveConnectionSegment& segment) {
                    if (segment.segmentType == WeaveSegmentType::MOVE)
                        ...
                        gcode.writeExtrusion(segment.to, speedBottom, extrusion_mm3_per_mm_flat, 0, PrintFeatureType::Skin);
                }
            );
    ...
    for (unsigned int layer_nr = 0; layer_nr < wireFrame.layers.size(); layer_nr++)
    {
        ...
        for (unsigned int part_nr = 0; part_nr < layer.connections.size(); part_nr++)
        {
            ...
            gcode.writeTypeComment(PrintFeatureType::OuterWall); // top
            {
                for (unsigned int segment_idx = 0; segment_idx < part.connection.segments.size(); segment_idx++)
                {
                    ...
                    {
                        gcode.writeExtrusion(segment.to, speedFlat, extrusion_mm3_per_mm_flat, 0, PrintFeatureType::OuterWall);
        ...
        writeFill(layer.roofs.roof_insets, layer.roofs.roof_outlines,
                ...
                [this](Wireframe2gcode&, WeaveConnectionSegment& segment) { // handle flat segments
                    ...
                    {
                        gcode.writeExtrusion(segment.to, speedFlat, extrusion_mm3_per_mm_flat, 0, PrintFeatureType::Skin);
                        gcode.writeDelay(flat_delay);
                    }
                });
...
void Wireframe2gcode::go_down(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    {
        gcode.writeExtrusion(segment.to, speedDown, extrusion_mm3_per_mm_connection, 0, PrintFeatureType::OuterWall);
    } else
    {
        ...
        gcode.writeExtrusion(up, speedDown*enlargement, extrusion_mm3_per_mm_connection / enlargement, 0, PrintFeatureType::OuterWall);
        gcode.writeExtrusion(to, speedDown*enlargement, extrusion_mm3_per_mm_connection / enlargement, 0, PrintFeatureType::OuterWall);
    }
    gcode.writeDelay(bottom_delay);
    if (up_dist_half_speed > 0)
    {

        gcode.writeExtrusion(Point3(0,0,up_dist_half_speed) + gcode.getPosition(), speedUp / 2, extrusion_mm3_per_mm_connection * 2, 0, PrintFeatureType::OuterWall);
    }
}
...
{
    WeaveConnectionSegment& segment = part.connection.segments[segment_idx];
    gcode.writeExtrusion(segment.to, speedUp, extrusion_mm3_per_mm_connection, 0, PrintFeatureType::OuterWall);
    Point3 next_vector;
...
void Wireframe2gcode::strategy_retract(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    if (lower_retract_start)
    {
        ...
        gcode.writeExtrusion(lower, speedUp, extrusion_mm3_per_mm_connection, 0, PrintFeatureType::OuterWall);
    ...
    {
        gcode.writeExtrusion(to, speedUp, extrusion_mm3_per_mm_connection, 0, PrintFeatureType::OuterWall);
        gcode.writeRetraction(retraction_config);
        gcode.writeTravel(to + Point3(0, 0, retract_hop_dist), speedFlat);
        gcode.writeDelay(top_retract_pause);
        if (after_retract_hop)
...
void Wireframe2gcode::strategy_compensate(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    gcode.writeExtrusion(newTop, speedUp * newLength / orrLength, extrusion_mm3_per_mm_connection * orrLength / newLength, 0, PrintFeatureType::OuterWall);
}
        ...
        case WeaveSegmentType::UP:
            {
                ...
                gcode.writeExtrusion(detoured, speedUp, extrusion_mm3_per_mm_connection, 0, PrintFeatureType::Skin);

            }
            break;
        case WeaveSegmentType::DOWN:
            gcode.writeExtrusion(segment.to, speedDown, extrusion_mm3_per_mm_connection, 0, PrintFeatureType::Skin);
...
void Wireframe2gcode::processSkirt()
{
    ...
    for (unsigned int poly_order_idx = 0; poly_order_idx < skirt.size(); poly_order_idx++)
    {
        ...
        for (unsigned int point_idx = 0; point_idx < poly.size(); point_idx++)
        {
            ...
            gcode.writeExtrusion(p, getSettingInMillimetersPerSecond("skirt_brim_speed"), getSettingInMillimeters("skirt_brim_line_width") * getSettingAsRatio("initial_layer_line_width_factor") * INT2MM(initial_layer_thickness), 0, PrintFeatureType::SkirtBrim);
        }
    }
}
