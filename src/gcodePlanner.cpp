...
GCodePath& GCodePlanner::addTravel(Point p, bool going_to_outer_wall)
{
    ...
    const bool always_retract = going_to_outer_wall && extr->getSettingBoolean("combing_retract_before_outer_wall");
...
void GCodePlanner::addPolygon(PolygonRef polygon, int start_idx, GCodePathConfig* config, WallOverlapComputation* wall_overlap_computation, coord_t wall_0_wipe_dist, bool spiralize, bool is_outer_wall)
{
    Point p0 = polygon[start_idx];
    addTravel(p0, is_outer_wall);
...
void GCodePlanner::addPolygonsByOptimizer(Polygons& polygons, GCodePathConfig* config, WallOverlapComputation* wall_overlap_computation, EZSeamType z_seam_type, Point z_seam_pos, coord_t wall_0_wipe_dist, bool spiralize, bool is_outer_wall)
{
    if (polygons.size() == 0)
    {
        return;
    ...
    {
        addPolygon(polygons[poly_idx], orderOptimizer.polyStart[poly_idx], config, wall_overlap_computation, wall_0_wipe_dist, spiralize, is_outer_wall);
    }
}
