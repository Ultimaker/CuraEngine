...
std::vector<coord_t> SkirtBrim::generatePrimaryBrim(std::vector<Offset>& all_brim_offsets, Polygons& covered_area, std::vector<Polygons>& allowed_areas_per_extruder)
{
    ...
    for (size_t offset_idx = 0; offset_idx < all_brim_offsets.size(); offset_idx++)
    {
        ...
        if (offset.is_last && total_length[offset.extruder_nr] < skirt_brim_minimal_length[offset.extruder_nr] && // This was the last offset of this extruder, but the brim lines don't meet minimal length yet
        ...
        )
        {
            ...
            all_brim_offsets.emplace_back(offset.inset_idx, external_polys_only[offset.extruder_nr], line_widths[offset.extruder_nr], offset.total_offset + line_widths[offset.extruder_nr], offset.inset_idx + 1, offset.extruder_nr, is_last);
...
Polygons SkirtBrim::getFirstLayerOutline(const int extruder_nr /* = -1 */)
{
    ...
    if (adhesion_type == EPlatformAdhesion::SKIRT)
    {
        ...
        first_layer_outline = first_layer_outline.unionPolygons(shields.offset(line_widths[reference_extruder_nr] / 2 // because the shield is printed *on* the stored polygons; not inside hteir area
    ...
    { // add brim underneath support by removing support where there's brim around the model
        ...
        if (storage.support.generated && primary_line_count > 0 && ! storage.support.supportLayers.empty() && (extruder_nr == -1 || extruder_nr == global_settings.get<int>("support_infill_extruder_nr")))
