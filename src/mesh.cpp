...
std::optional<IdFieldInfo> IdFieldInfo::from_aabb3d(const AABB3D& aabb)
{
    ...
    std::array<axis_span_t, 3> dif_per_axis = {
        std::make_tuple(IdFieldInfo::Axis::X, aabb.min_.x_, aabb.max_.x_),
...
void Mesh::setIdFieldInfo(const AABB3D& aabb)
{
    ...
    std::fprintf(stderr, "label %d: %ld - %ld // %d: %ld - %ld\n", dbgval.primary_axis_, dbgval.projection_field_.min_.X, dbgval.projection_field_.max_.X, dbgval.secondary_axis_, dbgval.projection_field_.min_.Y, dbgval.projection_field_.max_.Y);
