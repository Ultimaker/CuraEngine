...
CornerScoringCriterion::CornerScoringCriterion(const std::vector<StartCandidatePoint>& points, const EZSeamCornerPrefType corner_preference)
    : points_(points)
    ...
    , segments_sizes_(points.size())
{
    ...
    {
        const Point3LL& here = points_.at(i).position;
        const Point3LL& next = points_.at((i + 1) % points_.size()).position;
        const coord_t segment_size = (next - here).vSize();
...
double CornerScoringCriterion::cornerAngle(size_t vertex_index, const coord_t angle_query_distance) const
{
    ...
    const Point2LL here = points_.at(vertex_index).position.toPoint2LL();
...
Point2LL CornerScoringCriterion::findNeighbourPoint(size_t vertex_index, coord_t distance) const
{
    ...
    const Point2LL next_pos = points_.at((vertex_index + actual_delta + points_.size()) % points_.size()).position.toPoint2LL();
    ...
    if (travelled_distance > distance) [[likely]]
    {
        ...
        const Point2LL prev_pos = points_.at(prev).position.toPoint2LL();
