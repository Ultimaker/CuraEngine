...
class PolygonProximityLinker
{
    ...
    typedef std::unordered_map<WallOverlapPointLink, WallOverlapPointLinkAttributes, WallOverlapPointLink_Hasher> WallOverlapPointLinks; //!< The type of PolygonProximityLinker::overlap_point_links
    typedef std::unordered_map<Point, WallOverlapPointLinks::iterator> Point2Link; //!< The type of PolygonProximityLinker::point_to_link \warning mapping to iterators which might get invalidated!
    ...
    WallOverlapPointLinks overlap_point_links_endings; //!< mapping from each ending link to its attributes (which has a distance field equal to PolygonProximityLinker::line_width). Note that this is a separate map from PolygonProximityLinker::overlap_point_links, because that magically solved a bug .
    ...
    void findOverlapPoints(); //!< find the basic overlap links (for trapezoids) and record them into PolygonProximityLinker::overlap_point_links
    /*!
...
public:
     ...
     */
    PolygonProximityLinker(Polygons& polygons, int lineWidth);
