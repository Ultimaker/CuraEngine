...
std::optional<IdFieldInfo> IdFieldInfo::fromPointCloud(const std::vector<Point3LL>& points)
{
    ...
    // instead of doing full PCA (which is more efficient probably), for now just try to find the 'flattest' of 13 orientations (full grid: 9 * 3 = 27, then: (27 - 1) / 2 = 13 to get rid of the origin and mirrored directions)
    constexpr double D2 = 0.70710678118654752440084436210485;
    constexpr double D3 = 0.57735026918962576450914878050196;
    static std::vector<Point3D> normal_candidates =
        std::vector<Point3D>({
            Point3D(0, 0, 1),
            Point3D(0, 1, 0),
            Point3D(1, 0, 0),
    ...
    auto span_sizes =
        remapped_spans |
        ranges::view::transform([](const auto& key_value) { return std::make_pair(key_value.second.second - key_value.second.first, key_value.first); }) |
        ranges::to<std::vector<std::pair<double, int>>>();
    std::stable_sort(span_sizes.begin(), span_sizes.end(), [](const auto& a, const auto& b) { return a.first < b.first; });

    const auto& normal = normal_candidates[span_sizes.front().second];
    const auto& secondary = std::find_if(normal_candidates.begin(), normal_candidates.end(),
        [&normal](const auto& ax) { return normal.cross(ax).vSize() > CLOSE_1D; });
    if (secondary == normal_candidates.end() || *secondary == normal)
    {
        return std::nullopt;
    }
    const auto& primary = std::find_if(normal_candidates.begin(), normal_candidates.end(),
        [&normal, &secondary](const auto& ax) { return (*secondary).cross(ax).vSize() > CLOSE_1D && normal.cross(ax).vSize() > CLOSE_1D; });
