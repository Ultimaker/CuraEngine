...
auto MinimumSpanningTree::prim(std::vector<Point> vertices) const -> AdjacencyGraph_t
{
    ...
    while (result.size() < vertices_list.size()) // All of the vertices need to be in the tree at the end.
    {
        ...
        const auto closest = std::min_element(smallest_distance.begin(), smallest_distance.end(), [](const MapValue& a, const MapValue& b) { return a.second < b.second; });
...
std::vector<Point> MinimumSpanningTree::adjacentNodes(Point node) const
{
    ...
    if (adjacency_entry != adjacency_graph.end())
    {
        ...
        std::transform(edges.begin(), edges.end(), std::back_inserter(result), [&node](const Edge& e) { return (e.start == node) ? e.end : e.start; });
    }
    return result;
}
...
std::vector<Point> MinimumSpanningTree::vertices() const
{
    ...
    std::transform(adjacency_graph.begin(), adjacency_graph.end(), std::back_inserter(result), [](const MapValue& node) { return node.first; });
    return result;
}
