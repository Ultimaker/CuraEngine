...
#ifndef UTILS_SPARSE_CELL_GRID_3D_H
#define UTILS_SPARSE_CELL_GRID_3D_H
...
/*! \brief Sparse grid which efficiently stores data for some cubic cells in the grid
 * 
 * In contrast to a SparseGrid this class can only store a single item per cell
 * 
...
public:
    ...
    using GridMap = std::unordered_map<GridPoint3, Elem>;
     ...
     */
    void insertCell(GridPoint3 grid_loc, const Elem &elem);
...
SGI_TEMPLATE
ElemT& SGI_THIS::getCell(GridPoint3 loc, const ElemT& default_cell)
{
    auto it = m_grid.find(loc);
    if (it == m_grid.end())
    {
        m_grid.emplace(loc, default_cell);
    }
...
SGI_TEMPLATE
void SGI_THIS::insertCell(GridPoint3 grid_loc, const Elem &elem)
{
...
#endif // UTILS_SPARSE_CELL_GRID_3D_H
