...
void SkirtBrimAppender::process(PrintPlan* print_plan)
{
    ...
    const auto skirt_brim_extruder_nr = Application::getInstance().current_slice_->scene.current_mesh_group->settings.get<int>("skirt_brim_extruder_nr");
    ...
    std::map<ExtruderNumber, Shape> starting_outlines = generateStartingOutlines(print_plan, skirt_brim_extruder_nr, height);
    std::vector<Offset> offset_plan = generateBrimOffsetPlan(skirt_brim_extruder_nr, starting_outlines, first_extruder_plans);
    std::map<ExtruderNumber, Shape> allowed_areas_per_extruder = generateAllowedAreas(starting_outlines, adhesion_type, first_extruder_plans, skirt_brim_extruder_nr, storage_);

    generateSkirtBrim(storage_, adhesion_type, starting_outlines, offset_plan, allowed_areas_per_extruder, print_plan);
}

std::map<size_t, Shape> SkirtBrimAppender::generateStartingOutlines(const PrintPlan* print_plan, const int skirt_brim_extruder_nr, const size_t height)
{
    ...
    for (const LayerPlanPtr& layer_plan : print_plan->getOperationsAs<LayerPlan>())
    {
        if (layer_plan->getLayerIndex() < height)
        {
            ...
            {
                const ExtruderNumber target_extruder_nr = skirt_brim_extruder_nr >= 0 ? skirt_brim_extruder_nr : extruder_plan->getExtruderNr();
    ...
    for (auto &footprints : features_footprints)
    {
        starting_outlines[footprints.first] = Shape::unionShapes(footprints.second).getOutsidePolygons();
    }

    return starting_outlines;
}

std::vector<SkirtBrimAppender::Offset> SkirtBrimAppender::generateBrimOffsetPlan(const int skirt_brim_extruder_nr,
    ...
    const std::vector<ConstExtruderPlanPtr>& first_extruder_plans) const
{
    ...
    for (const ConstExtruderPlanPtr &extruder_plan : first_extruder_plans)
    {
        ...
        if ((skirt_brim_extruder_nr >= 0 && extruder_nr != skirt_brim_extruder_nr) || starting_outlines.at(extruder_nr).empty())
        {
        ...
        for (int line_idx = 0; line_idx < extruder_config.line_count_; line_idx++)
        {
            ...
            {
                all_brim_offsets
                    .emplace_back(&starting_outlines.at(extruder_nr), extruder_config.outside_polys_, extruder_config.inside_polys_, offset, offset, line_idx, extruder_nr, is_last);
            }
            else
            {
                all_brim_offsets
    ...
    const SliceDataStorage& storage) const
{
    ...
    for (const ConstExtruderPlanPtr& extruder_plan : first_extruder_plans)
    {
        ...
        allowed_areas = allowed_areas.difference(starting_outlines.at(extruder_nr).offset(extruder_config.gap_ - 50, ClipperLib::jtRound));
    }

    return allowed_areas_per_extruder;
}

void SkirtBrimAppender::generateSkirtBrim(const SliceDataStorage& storage,
    const EPlatformAdhesion adhesion_type,
    const std::map<ExtruderNumber, Shape>& starting_outlines,
    ...
    PrintPlan *print_plan) const
{
    ...
    for (size_t offset_idx = 0; offset_idx < offset_plan.size(); offset_idx++)
    {
        ...
        FeatureExtrusionPtr offset_extrusion = generateOffset(offset, starting_outlines, covered_area, allowed_areas_per_extruder, print_plan->getOperationsAs<LayerPlan>().front());
