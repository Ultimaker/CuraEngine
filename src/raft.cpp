...
void Raft::generate(SliceDataStorage& storage)
{
    ...
    {
        Polygons draft_shield_raft = storage.draft_protection_shield
...
coord_t Raft::getTotalThickness()
{
    ...
    return base_train.settings.get<coord_t>("raft_base_thickness") + interface_train.settings.get<size_t>("raft_interface_layers") * interface_train.settings.get<coord_t>("raft_interface_thickness")
