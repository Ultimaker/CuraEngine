...
TEST_P(GetDist2FromLineSegmentTest, GetDist2FromLineSegment)
{
    ...
    ASSERT_LE(std::fabs(sqrt(double(supposed_distance)) - sqrt(double(actual_distance2))), maximum_error)
        << "Line [" << line_start.X << ", " << line_start.Y << "] -- [" << line_end.X << ", " << line_end.Y << "], point [" << point.X
...
struct GetAngleParameters
{
    ...
    GetAngleParameters(Point a, Point b, Point c, double angle) : a(a), b(b), c(c), angle(angle)
    {
    }
};
...
TEST_P(GetAngleTest, GetAngle)
{
    ...
    ASSERT_LE(std::fabs(angle - supposed_angle), maximum_error)
        << "Corner in " << a << " - " << b << " - " << c << " was computed to have an angle of " << supposed_angle << " instead of "
        << angle << ".";
}
...
INSTANTIATE_TEST_CASE_P(
    ...
    GetAngleTest,
    testing::Values(GetAngleParameters(Point(-100, 0), Point(0, 0), Point(100, 1), 180), // Almost straight line.
...
TEST(PointIsLeftOfLineTest, LeftOfLine)
{
    ...
    ASSERT_TRUE(actual * supposed > 0 || (actual == 0 && supposed == 0))
        << "Point " << p << " was computed as lying " << ((supposed == 0) ? "on" : ((supposed < 0) ? "left" : "right")) << " the line from "
        << a << " to " << b << ", instead of " << ((actual == 0) ? "on" : ((actual < 0) ? "left" : "right"));
}
...
TEST_P(GetPointOnLineWithDistTest, GetPointOnLineWithDist)
{
    ...
    if (actual_returned)
    {
        EXPECT_TRUE(supposed_returned) << "Point " << p << " wasn't projected on (" << a << " - " << b << ") instead of projecting to "
                                       << actual_result << ".";
        EXPECT_LT(vSize2(actual_result - supposed_result), 10 * 10) << "Point " << p << " was projected on (" << a << " - " << b << ") to "
                                                                    << supposed_result << " instead of " << actual_result << ".";
        EXPECT_LT(std::abs(supposed_dist - dist), 10)
            << "Projection distance of " << p << " onto (" << a << " - " << b << ") was " << supposed_dist << " instead of " << dist << ".";
    }
    else
    {
...
INSTANTIATE_TEST_CASE_P(
    ...
    testing::Values(GetPointOnLineWithDistParameters(Point(110, 30), Point(0, 0), Point(100, 0), 50, Point(70, 0), true),
...
TEST_P(RotateAroundTest, RotateAround)
{
    ...
    ASSERT_LT(vSize(supposed_result - actual_result), 2)
        << "LinearAlg2D::rotateAround failed: Rotating " << point << " around " << origin << " for " << angle << " degrees resulted in "
        << supposed_result << " instead of expected " << actual_result << ".";
}
...
INSTANTIATE_TEST_CASE_P(RotateAroundInstantiation,
                        ...
                        testing::Values(RotateAroundParameters(Point(25, 30), Point(10, 17), 90, Point(-3, 32)), // 90 degrees rotation.
                                        ...
                                        RotateAroundParameters(Point(-67, 14),
                                                               Point(50, 50),
                                                               12,
                                                               Point(-57, -9)) // 12 degree rotation. Actually ends up at [-57, -9.5]!
                                        ));
