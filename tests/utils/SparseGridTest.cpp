...
struct GetNearestParameters
{
    ...
    GetNearestParameters(
        ...
        Point* result,
        const std::function<bool(const typename SparsePointGridInclusive<Point>::Elem&)>& filter = SparsePointGridInclusive<Point>::no_precondition)
        : registered_points(registered_points)
        , result(result)
        , filter(filter)
    {
...
INSTANTIATE_TEST_SUITE_P(
    ...
    GetNearestTest,
    testing::Values(
        GetNearestParameters(std::vector<Point>({ Point(95, 100), Point(103, 100), Point(200, 100) }), new Point(103, 100)), // Choose the nearest out of 3 points.
        GetNearestParameters(
            std::vector<Point>({ Point(95, 100), Point(98, 100), Point(106, 100) }),
            new Point(106, 100),
