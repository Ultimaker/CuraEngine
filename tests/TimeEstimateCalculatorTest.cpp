...
TEST_F(TimeEstimateCalculatorTest, StraightAngleNoJerk)
{
    ...
    const std::vector<Duration> result = calculator.calculate();
    EXPECT_NEAR(Duration(1.0 + first_cruise_distance / 50.0 + 1.0 + 1.0 + second_cruise_distance / 50.0 + second_decelerate_t),
                result[static_cast<size_t>(PrintFeatureType::Infill)],
                EPSILON);
}
...
TEST_F(TimeEstimateCalculatorTest, StraightAnglePartialJerk)
{
    ...
    const std::vector<Duration> result = calculator.calculate();
    EXPECT_NEAR(Duration(first_accelerate_t + first_cruise_distance / 50.0 + first_decelerate_t + second_accelerate_t
                         + second_cruise_distance / 50.0 + second_decelerate_t),
                result[static_cast<size_t>(PrintFeatureType::Infill)],
                EPSILON);
}
