...
void start_list_on_local_maximum(edge_list<T>& edges)
{
    ...
    while (edge != edges.end())
    {
        ...
        else if (! y_decreasing_before_last_horizontal && ! prev_edge_is_horizontal && edge_is_horizontal && (prev_edge->top == edge->top || prev_edge->top == edge->bot))
        {
...
bound<T> create_bound_towards_minimum(edge_list<T>& edges)
{
    ...
    while (next_edge != edges.end())
    {
        ...
        else if (! y_increasing_before_last_horizontal && ! edge_is_horizontal && next_edge_is_horizontal && (edge->bot == next_edge->top || edge->bot == next_edge->bot))
        {
...
bound<T> create_bound_towards_maximum(edge_list<T>& edges)
{
    ...
    while (next_edge != edges.end())
    {
        ...
        else if (! y_decreasing_before_last_horizontal && ! edge_is_horizontal && next_edge_is_horizontal && (edge->top == next_edge->top || edge->top == next_edge->bot))
        {
            y_decreasing_before_last_horizontal = true;
        }
        edge_is_horizontal = next_edge_is_horizontal;
...
void add_ring_to_local_minima_list(edge_list<T>& edges, local_minimum_list<T>& minima_list, polygon_type poly_type)
{
    ...
    while (! edges.empty())
    {
        ...
        if (to_max_first_non_horizontal == to_maximum.edges.end() || to_min_first_non_horizontal == to_minimum.edges.end())
        {
        ...
        if (! minimum_is_left)
        {
            ...
            minima_list.emplace_back(std::move(to_maximum), std::move(to_minimum), min_front.bot.y, lm_minimum_has_horizontal);
            if (! last_maximum)
            {
        ...
        else
        {
            ...
            to_minimum.winding_delta = -1;
            to_maximum.winding_delta = 1;
            minima_list.emplace_back(std::move(to_minimum), std::move(to_maximum), min_front.bot.y, lm_minimum_has_horizontal);
            if (! last_maximum)
            {
