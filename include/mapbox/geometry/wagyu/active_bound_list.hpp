...
inline std::basic_ostream<charT, traits>& operator<<(std::basic_ostream<charT, traits>& out, const active_bound_list<T>& bnds)
{
    std::size_t c = 0;
...
struct bound_insert_location
{
    ...
    bound_insert_location(bound<T> const& b)
        : bound2(b)
    {
    }
    ...
    bool operator()(bound_ptr<T> const& b)
    {
        ...
        if (values_are_equal(bound2.current_x, bound1.current_x))
        {
            ...
            {
                return less_than(static_cast<double>(bound2.current_edge->top.x), get_current_x(*(bound1.current_edge), bound2.current_edge->top.y));
            }
            else
            {
                return greater_than(static_cast<double>(bound1.current_edge->top.x), get_current_x(*(bound2.current_edge), bound1.current_edge->top.y));
            }
        }
        else
        {
...
template<typename T>
void set_winding_count(active_bound_list_itr<T> bnd_itr, active_bound_list<T>& active_bounds, fill_type subject_fill_type, fill_type clip_fill_type)
{
...
template<typename T>
void insert_lm_left_and_right_bound(
    bound<T>& left_bound,
    bound<T>& right_bound,
...
template<typename T>
void insert_local_minima_into_ABL(
    T const bot_y,
    ...
    fill_type clip_fill_type)
{
    while (current_lm != minima_sorted.end() && bot_y == (*current_lm)->y)
    {
        ...
        insert_lm_left_and_right_bound(left_bound, right_bound, active_bounds, rings, scanbeam, cliptype, subject_fill_type, clip_fill_type);
        ++current_lm;
    }
}

template<typename T>
void insert_horizontal_local_minima_into_ABL(
    T const top_y,
    local_minimum_ptr_list<T> const& minima_sorted,
    local_minimum_ptr_list_itr<T>& current_lm,
    active_bound_list<T>& active_bounds,
