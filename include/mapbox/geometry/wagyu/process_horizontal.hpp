    ...
    fill_type clip_fill_type)
{
    ...
    while (bnd != active_bounds.end())
    {
        ...
        while (hp_itr != rings.hot_pixels.end() && hp_itr->y == scanline_y && hp_itr->x < wround<T>((*bnd)->current_x) && hp_itr->x < (*horz_bound)->current_edge->top.x)
        {
            if ((*horz_bound)->ring)
            {
        ...
        if (wround<T>((*bnd)->current_x) == (*horz_bound)->current_edge->top.x && (*horz_bound)->next_edge != (*horz_bound)->edges.end()
        ...
        if ((*horz_bound)->ring)
        {
            add_point_to_ring(*(*horz_bound), mapbox::geometry::point<T>(wround<T>((*bnd)->current_x), scanline_y), rings);
        }
        ...
        if (is_maxima_edge && bnd == bound_max_pair)
        {
            ...
            {
                add_local_maximum_point(*(*horz_bound), *(*bound_max_pair), (*horz_bound)->current_edge->top, rings, active_bounds);
            }
    ...
    if ((*horz_bound)->ring)
    {
        while (hp_itr != rings.hot_pixels.end() && hp_itr->y == scanline_y && hp_itr->x < (*horz_bound)->current_edge->top.x)
        {
            add_point_to_ring(*(*horz_bound), *hp_itr, rings);
            ++hp_itr;
        }
    ...
    fill_type clip_fill_type)
{
    ...
    while (hp_itr_fwd != rings.hot_pixels.end() && (hp_itr_fwd->y < scanline_y || (hp_itr_fwd->y == scanline_y && hp_itr_fwd->x < (*horz_bound_fwd)->current_edge->top.x)))
    {
        ++hp_itr_fwd;
    }
    ...
    while (bnd != active_bounds.rend())
    {
        ...
        while (hp_itr != rings.hot_pixels.rend() && hp_itr->y == scanline_y && hp_itr->x > wround<T>((*bnd)->current_x) && hp_itr->x > (*horz_bound)->current_edge->top.x)
        {
            if ((*horz_bound)->ring)
            {
                add_point_to_ring(*(*horz_bound), *hp_itr, rings);
    ...
    fill_type clip_fill_type)
{
    ...
    {
        return process_horizontal_left_to_right(scanline_y, horz_bound, active_bounds, rings, scanbeam, cliptype, subject_fill_type, clip_fill_type);
    }
    else
    {
        return process_horizontal_right_to_left(scanline_y, horz_bound, active_bounds, rings, scanbeam, cliptype, subject_fill_type, clip_fill_type);
    }
}
    ...
    fill_type clip_fill_type)
{
    for (auto bnd_itr = active_bounds.begin(); bnd_itr != active_bounds.end();)
    {
        ...
        {
            bnd_itr = process_horizontal(scanline_y, bnd_itr, active_bounds, rings, scanbeam, cliptype, subject_fill_type, clip_fill_type);
        }
        else
        {
            ++bnd_itr;
