...
template<typename T>
void hot_pixel_set_left_to_right(T y, T start_x, T end_x, bound<T>& bnd, ring_manager<T>& rings, hot_pixel_itr<T>& itr, hot_pixel_itr<T>& end, bool add_end_point)
{
...
template<typename T>
void hot_pixel_set_right_to_left(T y, T start_x, T end_x, bound<T>& bnd, ring_manager<T>& rings, hot_pixel_rev_itr<T>& itr, hot_pixel_rev_itr<T>& end, bool add_end_point)
{
    T x_min = get_edge_min_x(*(bnd.current_edge), y);
...
template<typename T>
void insert_hot_pixels_in_path(bound<T>& bnd, mapbox::geometry::point<T> const& end_pt, ring_manager<T>& rings, bool add_end_point)
{
...
template<typename T>
void add_first_point(bound<T>& bnd, active_bound_list<T>& active_bounds, mapbox::geometry::point<T> const& pt, ring_manager<T>& rings)
{
...
template<typename T>
void add_point(bound<T>& bnd, active_bound_list<T>& active_bounds, mapbox::geometry::point<T> const& pt, ring_manager<T>& rings)
{
    if (bnd.ring == nullptr)
    {
...
template<typename T>
void add_local_minimum_point(bound<T>& b1, bound<T>& b2, active_bound_list<T>& active_bounds, mapbox::geometry::point<T> const& pt, ring_manager<T>& rings)
{
...
bool first_is_bottom_point(const_point_ptr<T> btmPt1, const_point_ptr<T> btmPt2)
{
    ...
    if (values_are_equal(std::max(dx1p, dx1n), std::max(dx2p, dx2n)) && values_are_equal(std::min(dx1p, dx1n), std::min(dx2p, dx2n)))
    {
        std::size_t s = 0;
...
template<typename T>
void add_local_maximum_point(bound<T>& b1, bound<T>& b2, mapbox::geometry::point<T> const& pt, ring_manager<T>& rings, active_bound_list<T>& active_bounds)
{
    ...
    do
    {
        ...
        if ((op->y < pt.y) != (op->next->y < pt.y))
        {
            if (op->x >= pt.x)
            {
                ...
                {
                    double d
                        = static_cast<double>(op->x - pt.x) * static_cast<double>(op->next->y - pt.y) - static_cast<double>(op->next->x - pt.x) * static_cast<double>(op->y - pt.y);
                    if (value_is_zero(d))
                    {
                        return point_on_polygon;
                    }
...
point_in_polygon_result point_in_polygon(mapbox::geometry::point<double> const& pt, point_ptr<T> op)
{
    ...
    do
    {
        ...
        {
            if (values_are_equal(op_next_x, pt.x) || (values_are_equal(op_y, pt.y) && ((op_next_x > pt.x) == (op_x < pt.x))))
            {
                return point_on_polygon;
            }
        }
...
{
    return (box2.max.x >= box1.max.x && box2.max.y >= box1.max.y && box2.min.x <= box1.min.x && box2.min.y <= box1.min.y);
}
