...
inline std::basic_ostream<charT, traits>& operator<<(std::basic_ostream<charT, traits>& out, const std::unordered_multimap<ring_ptr<T>, point_ptr_pair<T>>& dupe_ring)
{
...
{
    {
        ...
        for (auto& it = range.first; it != range.second;)
        {
            ...
            if (it_ring2 == ring_origin && (ring_parent == it_ring2 || ring_parent == it_ring2->parent) && *prev_pt != *it->second.op2 && *orig_pt != *it->second.op2)
            {
    ...
    for (auto& it = range.first; it != range.second && it != dupe_ring.end() && it->first == ring_search; ++it)
    {
        ...
        if (visited.count(it_ring) > 0 || it_ring == nullptr || (ring_parent != it_ring && ring_parent != it_ring->parent) || value_is_zero(it_ring->area())
            || *prev_pt == *it->second.op2)
        {
            continue;
        }
        if (find_intersect_loop(dupe_ring, iList, ring_parent, ring_origin, it_ring, visited, orig_pt, it->second.op2, rings))
        {
            iList.emplace_front(ring_search, it->second);
            return true;
        }
...
template<typename T>
void correct_repeated_points(ring_manager<T>& manager, ring_vector<T>& new_rings, point_vector_itr<T> const& begin, point_vector_itr<T> const& end)
{
...
template<typename T>
void reassign_children_if_necessary(ring_ptr<T> new_ring, ring_ptr<T> sibling_ring, ring_manager<T>& manager, ring_vector<T>& new_rings)
{
...
template<typename T>
void process_single_intersection(std::unordered_multimap<ring_ptr<T>, point_ptr_pair<T>>& connection_map, point_ptr<T> op_j, point_ptr<T> op_k, ring_manager<T>& manager)
{
    ...
    if (iList.empty())
    {
        ...
        visited.insert(ring_search);
        // Check for connection through chain of other intersections
        for (auto& it = range.first; it != range.second && it != connection_map.end() && it->first == ring_search; ++it)
        {
            ring_ptr<T> it_ring = it->second.op2->ring;
            if (it_ring != ring_search && *op_origin_2 != *it->second.op2 && it_ring != nullptr && (ring_parent == it_ring || ring_parent == it_ring->parent)
                && ! value_is_zero(it_ring->area()) && find_intersect_loop(connection_map, iList, ring_parent, ring_origin, it_ring, visited, op_origin_2, it->second.op2, manager))
            {
                found = true;
                iList.emplace_front(ring_search, it->second);
                break;
...
template<typename T>
void correct_collinear_repeats(ring_manager<T>& manager, point_vector_itr<T> const& begin, point_vector_itr<T> const& end)
{
    for (auto itr1 = begin; itr1 != end; ++itr1)
    {
        if ((*itr1)->ring == nullptr)
