...
struct intersect_list_sorter
{
    inline bool operator()(intersect_node<T> const& node1, intersect_node<T> const& node2)
    {
        ...
        {
            return (node2.bound1->winding_count2 + node2.bound2->winding_count2) > (node1.bound1->winding_count2 + node1.bound2->winding_count2);
        }
    }
};
...
struct on_intersection_swap
{
    ...
    on_intersection_swap(intersect_list<T>& i)
        : intersects(i)
    {
    }
...
{
    bubble_sort(active_bounds.begin(), active_bounds.end(), intersection_compare<T>(), on_intersection_swap<T>(intersects));
}

template<typename T>
void intersect_bounds(
    bound<T>& b1,
    bound<T>& b2,
    ...
    active_bound_list<T>& active_bounds)
{
    ...
    {
        if ((b1Wc != 0 && b1Wc != 1) || (b2Wc != 0 && b2Wc != 1) || (b1.poly_type != b2.poly_type && cliptype != clip_type_x_or))
        {
    ...
    else if ((b1Wc == 0 || b1Wc == 1) && (b2Wc == 0 || b2Wc == 1))
    {
        ...
        else if (b1Wc == 1 && b2Wc == 1)
        {
            ...
            case clip_type_difference:
                if (((b1.poly_type == polygon_type_clip) && (b1Wc2 > 0) && (b2Wc2 > 0)) || ((b1.poly_type == polygon_type_subject) && (b1Wc2 <= 0) && (b2Wc2 <= 0)))
                {
                    add_local_minimum_point(b1, b2, active_bounds, pt, rings);
                }
                break;
    ...
    active_bound_list<T>& active_bounds)
{
    for (auto node_itr = intersects.begin(); node_itr != intersects.end(); ++node_itr)
    {
        ...
        intersect_bounds(*(node_itr->bound1), *(node_itr->bound2), pt, cliptype, subject_fill_type, clip_fill_type, rings, active_bounds);
        std::iter_swap(b1, b2);
    }
}
...
template<typename T>
void process_intersections(T top_y, active_bound_list<T>& active_bounds, clip_type cliptype, fill_type subject_fill_type, fill_type clip_fill_type, ring_manager<T>& rings)
{
