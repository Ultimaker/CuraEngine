...
template<typename T>
active_bound_list_itr<T> do_maxima(
    active_bound_list_itr<T>& bnd,
    ...
    active_bound_list<T>& active_bounds)
{
    ...
    while (bnd_next != active_bounds.end() && bnd_next != bndMaxPair)
    {
        ...
        skipped = true;
        intersect_bounds(*(*bnd), *(*bnd_next), (*bnd)->current_edge->top, cliptype, subject_fill_type, clip_fill_type, manager, active_bounds);
...
template<typename T>
void process_edges_at_top_of_scanbeam(
    T top_y,
    active_bound_list<T>& active_bounds,
    ...
    fill_type clip_fill_type)
{
    for (auto bnd = active_bounds.begin(); bnd != active_bounds.end();)
    {
        ...
        if (is_maxima_edge)
        {
            ...
            is_maxima_edge = ((bnd_max_pair == active_bounds.end() || ! current_edge_is_horizontal<T>(bnd_max_pair)) && is_maxima(bnd_max_pair, top_y));
            if (is_maxima_edge)
            {
    ...
    insert_horizontal_local_minima_into_ABL(top_y, minima_sorted, current_lm, active_bounds, manager, scanbeam, cliptype, subject_fill_type, clip_fill_type);
