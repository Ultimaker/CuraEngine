...
#ifndef GEOMETRY_POLYGONS_H
#define GEOMETRY_POLYGONS_H
...
public:
    ...
    Polygons(const std::vector<ClipperLib::Path>& paths)
     ...
     */
    Polygons removePolygon(const Polygons& to_be_removed, int same_distance = 0) const;
    ...
    Polygons offsetMulti(const std::vector<coord_t>& offset_dists) const;
