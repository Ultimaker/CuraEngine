...
void SGI_THIS::insert(const Elem &elem)
{
    ...
    const std::function<bool(const GridPoint)> cell_insert_func
    {
        [&elem, this](const GridPoint grid_loc)
        {
            this->m_grid.emplace(grid_loc, elem);
            return true;
        }
    };
    SparseGrid<ElemT>::processLineCells(line, cell_insert_func);
}
