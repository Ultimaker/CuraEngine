...
protected:
    ...
    constexpr static Ratio max_gap = 0.5; //!< The maximum allowed gap between lines that get connected, in multiples of the local line width. Allows connections inside corners
    ...
    coord_t getSpace(const PolygonConnection<Polygonal>& connection) const
    {
        const coord_t from_width = interpolateWidth(
            ...
            (*connection.from_poly)[connection.from_segment],
            (*connection.from_poly)[(connection.from_segment + 1) % connection.from_poly->size()]);
        const coord_t to_width
            = interpolateWidth(connection.to_point, (*connection.to_poly)[connection.to_segment], (*connection.to_poly)[(connection.to_segment + 1) % connection.to_poly->size()]);
     ...
     */
    template<typename Polygonal>
    std::optional<std::pair<Point, size_t>>
        walkUntilDistanceFromLine(const Polygonal& poly, const size_t start_index, const coord_t distance, const Point& line_a, const Point& line_b, const short direction)
    {
    ...
    std::optional<PolygonConnection<Polygonal>> getSecondConnection(PolygonConnection<Polygonal>& first, const coord_t adjacent_distance)
    {
        ...
        std::optional<std::pair<Point, size_t>> from_forward_intersection
            = walkUntilDistanceFromLine(*first.from_poly, first.from_segment, adjacent_distance, first.from_point, first.to_point, +1);
        std::optional<std::pair<Point, size_t>> from_backward_intersection
            = walkUntilDistanceFromLine(*first.from_poly, first.from_segment, adjacent_distance, first.from_point, first.to_point, -1);
        std::optional<std::pair<Point, size_t>> to_forward_intersection
            = walkUntilDistanceFromLine(*first.to_poly, first.to_segment, adjacent_distance, first.from_point, first.to_point, +1);
        std::optional<std::pair<Point, size_t>> to_backward_intersection
            = walkUntilDistanceFromLine(*first.to_poly, first.to_segment, adjacent_distance, first.from_point, first.to_point, -1);
        ...
        for (const std::optional<std::pair<Point, size_t>>& from_intersection : { from_forward_intersection, from_backward_intersection })
        {
            ...
            for (const std::optional<std::pair<Point, size_t>>& to_intersection : { to_forward_intersection, to_backward_intersection })
            {
                ...
                PolygonConnection<Polygonal>
                    connection(first.from_poly, from_intersection->second, from_intersection->first, first.to_poly, to_intersection->second, to_intersection->first);
    ...
    void connectPolygonsAlongBridge(const PolygonBridge<Polygonal>& bridge, Polygonal& result)
    {
        ...
        const coord_t b_from_width
            = interpolateWidth(bridge.b.from_point, (*bridge.b.from_poly)[bridge.b.from_segment], (*bridge.b.from_poly)[(bridge.b.from_segment + 1) % from_size]);
        ...
        const coord_t a_from_width
            = interpolateWidth(bridge.a.from_point, (*bridge.b.from_poly)[bridge.a.from_segment], (*bridge.b.from_poly)[(bridge.a.from_segment + 1) % from_size]);
